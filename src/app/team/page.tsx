 ...
 * from a real timestamp — prospects.created_at, prospect_contacts.sent_at
 * and .responded_at, prospect_notes.created_at — so the numbers are
 * activity, not a snapshot guess. Attribution is by the prospect's owner:
 * each card is one rep's book of business and what happened inside it.
 *
 * The standalone page the dead /team nav always promised.
 */
...
export default async function TeamPage({
  ...
  const [
    ...
    sql<CountRow>`
      ...
      GROUP BY owner_id`,
    // Prospects whose first logged touch landed in the window — i.e. the
    // rep started actively contacting them (research → tracking).
    sql<CountRow>`
      SELECT p.owner_id, COUNT(*)::int AS n
      FROM (
        SELECT prospect_id, MIN(sent_at) AS first_at
  ...
  const started = mapOf(startedRows);
  ...
  const reports: RepReport[] = repRows.map((r) => {
    ...
    const st = started.get(r.user_id) ?? 0;
  ...
  const team = reports.reduce(
    ...
    { added: 0, started: 0, touches: 0, replies: 0 },
  );
        ...
        <main className="app-shell-main" style={{ flex: 1 }}>
          <div style={{ maxWidth: 900, margin: '0 auto' }}>
            ...
            <p style={{ color: 'var(--text-mid)', marginBottom: '1.25rem', maxWidth: '58ch' }}>
              ...
              stands now. Across the team: {team.added} added, {team.started} started
              contacting, {team.touches} touches, {team.replies} repl
            ...
            >
              Windowed metrics count real timestamps. &ldquo;Started contacting&rdquo;
              is the first touch logged for a prospect — the move from research into
              the active cycle. The pipeline snapshot is current, not windowed.
            </p>
          </div>
        </main>
    ...
    rep.activity === 0
      ...
      : 'Over the last ' +
        ...
        [
          ...
          rep.started > 0 &&
            `started contacting ${rep.started}`,
  ...
  return (
    <div className="surface-card">
      ...
      >
        ...
        <Stat n={rep.started} label="Started" />
